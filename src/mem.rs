...
impl ReferenceCount {
    ...
    pub fn release<U: ?Sized>(&mut self, ptr: *mut U) {
        unsafe {
            if !self.holder.is_null() && !ptr.is_null() {
                *self.holder -= 1;
                if *self.holder == 0 {
                ...
                // no holders and a valid pointer, drop.
                mem::drop(Box::from_raw(ptr));
            }
        }
    }
...
impl<T> Shared<T> {
    pub fn new(value: T) -> Self {
        Self(::alloc::rc::Rc::new(Locked::new(true, Box::new(value))))
    }
}
/*
impl< T> Shared<T> where T: I {
    pub fn coerce_dyn(self) -> Shared<dyn I> {
        self as ::alloc::rc::Rc<Locked<Box<dyn I>>>
    }
} */

impl<'borrow, T: ?Sized> Shared<T> {
    pub fn get(&'borrow self) -> LockGuard<'borrow, Box<T>> {
        self.0.get_locked()
    }
}
