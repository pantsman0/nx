...
pub struct Directory {
    dir_obj: Box<dyn super::Directory>,
...
impl Directory {
    ...
    pub fn new(dir_obj: Box<dyn super::Directory>) -> Self {
        Self {
            dir_obj,
...
impl IDirectory for Directory {
    fn read(&mut self, out_entries: sf::OutMapAliasBuffer<fsp::DirectoryEntry>) -> Result<u64> {
        self.dir_obj.read(out_entries.get_mut_slice())
    }
...
impl File {
    ...
    pub fn new(file_obj: Box<dyn super::File>) -> Self {
        Self {
            file_obj,
            dummy_session: sf::Session::new()
        }
...
impl IFile for File {
    fn read(&mut self, option: fsp::FileReadOption, offset: usize, size: usize, buf: sf::OutNonSecureMapAliasBuffer<u8>) -> Result<usize> {
        self.file_obj.read(offset, buf.get_address(), size.min(buf.get_size()), option)
    }

    fn write(&mut self, option: fsp::FileWriteOption, offset: usize, size: usize, buf: sf::InNonSecureMapAliasBuffer<u8>) -> Result<()> {
        self.file_obj.write(offset, buf.get_address(), size.min(buf.get_size()), option)
    }
    ...
    fn operate_range_with_buffer(&mut self, operation_id: fsp::OperationId, offset: usize, size: usize, in_buf: sf::InNonSecureMapAliasBuffer<u8>, out_buf: sf::OutNonSecureMapAliasBuffer<u8>) -> Result<()> {
        self.file_obj.operate_range_with_buffer(operation_id, offset, size, in_buf.get_address(), in_buf.get_size(), out_buf.get_address(), out_buf.get_size())
    }
}
...
pub struct FileSystem {
    fs_obj: Box<dyn super::FileSystem>,
    dummy_session: sf::Session
}
...
impl FileSystem {
    ...
    pub fn new(fs_obj: Box<dyn super::FileSystem>) -> Self {
        Self {
            fs_obj,
            dummy_session: sf::Session::new()
        }
...
impl IFileSystem for FileSystem {
    ...
    fn rename_file(&mut self, old_path_buf: sf::InFixedPointerBuffer<fsp::Path>, new_path_buf: sf::InFixedPointerBuffer<fsp::Path>) -> Result<()> {
        ...
        self.fs_obj.rename_file(old_fs_path, new_fs_path)
    }
    ...
    fn open_file(&mut self, mode: fsp::FileOpenMode, path_buf: sf::InFixedPointerBuffer<fsp::Path>) -> Result<::alloc::boxed::Box<dyn fsp::IFile>> {
        let fs_path = path_buf.get_var().get_string()?;
        let file_obj = self.fs_obj.open_file(fs_path, mode)?;
        Ok(Box::new(File::new(file_obj)))
    }
    ...
    fn open_directory(&mut self, mode: fsp::DirectoryOpenMode, path_buf: sf::InFixedPointerBuffer<fsp::Path>) -> Result<::alloc::boxed::Box<dyn fsp::IDirectory>> {
        let fs_path = path_buf.get_var().get_string()?;
        let dir_obj = self.fs_obj.open_directory(fs_path, mode)?;
        Ok(Box::new(Directory::new(dir_obj)))
    }
    ...
    fn query_entry(&mut self, path_buf: sf::InFixedPointerBuffer<fsp::Path>, query_id: fsp::QueryId, in_buf: sf::InNonSecureMapAliasBuffer<u8>, out_buf: sf::OutNonSecureMapAliasBuffer<u8>) -> Result<()> {
        let fs_path = path_buf.get_var().get_string()?;
        self.fs_obj.query_entry(fs_path, query_id, in_buf.get_address(), in_buf.get_size(), out_buf.get_address(), out_buf.get_size())
    }
}
